...
//        self.l = self.layers_cfg.len() + 1; // Makes sure that the `l` variable is always kept even with the layers_cfg
    ...
    fn backwards_pass(&mut self, num: usize, batch_size: usize) {
        ...
        for layer in { 0..bravo }.rev() {
            ...
            match activation_fn {
                ...
                "tanh" => {
                    self.z[layer].par_mapv_inplace(|x| tanh_prime(x));
                }
                "linear" => {
                    self.z[layer].par_mapv_inplace(|x| linear_prime(x));
                }
                _ => panic!(format!(
                    "This activation function ({}) is not supported",
                    activation_fn
    ...
    pub fn predict(&mut self, input: Array2<f32>) -> Array2<f32> {
        self.input = input;
        self.forward_pass(0, self.input.nrows());
        self.a.last().unwrap().clone()
    }

    pub fn single_training_batch(&mut self, input: Array2<f32>, output: Array2<f32>, number_of_examples: usize) {
